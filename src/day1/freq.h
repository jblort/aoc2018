...
namespace freq {
    ...
    inline int firstRepeatingFreq() {
        ...
        while(true) {
            for (auto num : input) {
                freq += num;
                if (std::find(std::begin(seenFreqs), std::end(seenFreqs), freq) != std::end(seenFreqs)) {
                    return freq;
                } else {
